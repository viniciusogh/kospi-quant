...
- **주문 직전에 현재가를 다시 잡는다.** propose 는 장 시작 전에 도는데 그때 시세 API 는 전일 종가를 준다 —
  리포트의 '진입가' 와 같은 값이다. 그대로 지정가로 걸면 갭상승한 모멘텀 종목은 체결이 안 되는데
  알림은 "주문 전송 완료" 로 나간다. poll 이 주문 직전에 재조회하고, 승인가 대비 `TRADE_MAX_DRIFT`(3%)
  넘게 벌어지면 **사지 않고 알린다** — 규환님이 승인한 가격과 다른 가격에 임의로 사지 않는다
- **이 봇의 `getUpdates` 는 auto_trade 만 부른다.** 텔레그램은 offset 을 올리는 순간 업데이트를 소비하므로
  같은 토큰에 폴러가 둘이면 서로 뺏어간다 — 규환님이 누른 승인 콜백이 다른 폴러로 빨려 들어가 매수가
  안 나가거나, 누가 `setWebhook` 을 걸면 이쪽이 409 로 죽는다. **둘 다 무음이다.**
  다른 스크립트(alert.py·리포트 전송)는 `sendMessage` 만 쓴다. 버튼이 필요하면 봇을 따로 판다
  (2026-09-06 합의)
- **HTML parse_mode 를 쓰고 종목명은 반드시 `E()` 로 이스케이프한다.** 코스피 2,453 종목 중 139개에
  `&` 가 있다(KT&G·F&F·신세계 I&C). 안 하면 400 이 나고 알림이 통째로 사라진다.
  MarkdownV2 는 `.` `-` `(` `)` `!` 가 전부 이스케이프 대상이라 한국어 종목명·수익률에서 사고가 잦다
- **momentum CSV 를 읽지 않는다.** 입력은 노션 Report DB 뿐이다 — `latest_momentum_reco_v20g.csv` 는
  게이트가 걸린 날에도 10종목이 들어 있어(파일 쓰기가 cash_mode 판정보다 앞) 그걸 후보로 쓰면
  '전량 현금 권장' 인 날에 주문이 나간다
...
## 일일 아카이브 (`daily_archive.py`) — 2026-09-04·06 규환 결정

**평일 18:00 + 주말·공휴일 18:00** (launchd `com.vinicius.daily-archive`, Weekday 0~6).
21:00 이던 것을 앞당겼다 — 모멘텀 산출물이 17:30~17:40 에 올라오는 것을 실측했고,
아침 7시에 열었을 때 이미 있어야 그날 매수 판단에 쓴다.

- **페이지 날짜 = 데이터가 나온 날.** 매수일이 아니다. 그 대신 라벨로 밝힌다 —
  「9/7(월)에 살 종목 — 대한항공(003490)」. 장 마감 후 만드니 추천은 **다음 거래일용**이다
- **진입가 = 그날 종가.** 과거 60건과 기준을 맞추기 위함이고, 그 가격으로는 실제로 못 산다.
  페이지에 "실제 매수는 M/D 시가부터 가능하다" 를 명시한다. 지정가로 그대로 쓰면 안 된다
- **휴장일에도 페이지를 만든다.** 시장은 쉬어도 유튜브는 올라온다. 시세가 없으므로
  **종목 추천을 하지 않는다** — 낡은 후보로 추천하면 성과 기록이 오염된다.
  그 행은 `추천종목`·`진입가` 가 비고 `게이트` 가 **"휴장"** 이다(기존 값은 통과/미충족뿐이었다).
  휴장 판정은 `portfolio.market_open_today()` (토스 장운영 API) — 주말 + 대체공휴일
- **입력이 낡으면 드러낸다.** 1영업일은 📅 노란 안내(휴장일일 수 있다), 2영업일 이상은 🚨 빨간 경고.
  판정은 영업일·시각 기준 — 평일 17:30 이후면 오늘, 그 전이면 전 영업일이 최신이다
- **실행 직전에 `git pull`.** 이 저장소를 당기는 스케줄이 아무 데도 없어 로컬이 35커밋
  뒤쳐졌고, 9/3 아카이브가 9/2 후보로 추천했다(2026-09-04 발견). ETL 이 당기는
  `~/Desktop/퀀트스코어` 는 git 저장소가 아니다. pull 실패 시 `git rebase --abort` 로
  원상복구한다 — 충돌 잔재가 남으면 이후 **모든** pull 이 막혀 영구히 낡은 데이터로 돈다

### 원문 복사의 깊이 제약 (재발 주의)

노션은 **요청당 중첩 2단계**까지만 만든다. 대시보드를 그대로 복사하면 3단계 이하가 사라진다.

- 유튜브: 페이지 → 유튜브토글 → 채널토글 → **영상토글 → 분석내용** 이 버려져
  "빈 토글입니다" 만 남았다 (9/4 실측: 토글 25개 중 20개가 껍데기)
- 모멘텀: 종목토글 안 **분기실적표**가 문단으로 대체됐다
- → 복사 시점에 자식을 떠서 **경로((서명, 등장순서), ...)** 로 보관하고, 페이지 생성 후
  그 경로로 사본에서 찾아 붙인다. **붙일 때 원본을 다시 읽지 말 것** — 대시보드는
  15분마다 덮어써져 형제 순서가 틀어지면 엉뚱한 토글에 붙는다(코덱스 지적)
- 노션 3rps 제한: 0.34초 throttle + 429 Retry-After 준수. 예산 `ARCHIVE_HTTP_BUDGET`(600)
- 복사 중 잃은 것은 `_lost()` 로 모아 페이지 하단 ⚠️ 콜아웃으로 띄운다.
  **조용히 빠지면 안 된다** — 이 결함들이 전부 무음이라 며칠씩 몰랐다
