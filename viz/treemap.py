...
# 순환매 판정: 오늘 방향 × 20일 추세. 강세 목록에 숫자를 6개씩 나열하면 안 읽힌다(사용자 지적)
# → "지금 사도 되는 자리인가" 한 단어로 압축한다.
def verdict(today, d20):
    if today > 0:
        return ("추세 지속", UP_STEPS[2]) if d20 > 0 else ("반등 시작", UP_STEPS[3])
    return ("조정 중", DOWN_STEPS[1]) if d20 > 0 else ("약세 지속", DOWN_STEPS[2])


def _num_ink(pct):
    """표 글자색. color_for 는 타일 배경용이라 중립 단계가 배경 대비 1.5 로 묻힌다."""
    c = color_for(pct)
    return INK if c == NEUTRAL else c


def render_table(rows, asof, out_path, w=820):
    """rows: [(섹터, 오늘, 20일, 주도주명, 주도주등락)] — 강세 먼저, 약세 나중.

    모바일에서 노션 표는 열이 잘리고 목록은 숫자 벽이 된다 → 이미지 한 장으로.
    폭을 좁게(820) 잡아야 휴대폰에서 축소돼도 글자가 읽힌다.
    """
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    RH, HEAD, PAD = 54, 104, 26
    h = HEAD + RH * len(rows) + 28
    fig = plt.figure(figsize=(w / 100, h / 100), dpi=100, facecolor=SURFACE)
    ax = fig.add_axes([0, 0, 1, 1]); ax.set_xlim(0, w); ax.set_ylim(0, h); ax.axis("off")

    ax.text(PAD, h - 38, "섹터 추세", fontsize=23, fontweight="bold", color=INK, va="center")
    ax.text(w - PAD, h - 38, asof, fontsize=15, color=SUBINK, va="center", ha="right")
    ax.text(PAD, h - 70, "오늘 등락 × 20일 추세로 순환매 위치를 판정", fontsize=13, color=SUBINK, va="center")
    ax.plot([PAD, w - PAD], [h - HEAD + 16] * 2, color=NEUTRAL, lw=1.2)

    for i, (sec, td, d20, lname, lchg) in enumerate(rows):
        y = h - HEAD - RH * i - RH / 2 + 12
        if i:
            ax.plot([PAD, w - PAD], [y + RH / 2] * 2, color="#363a47", lw=0.8)
        vt, vc = verdict(td, d20)
        ax.text(PAD, y, sec, fontsize=20, fontweight="bold", color=INK, va="center")
        ax.text(300, y, f"{td*100:+.1f}%", fontsize=21, fontweight="bold",
                color=_num_ink(td * 100), va="center", ha="right")
        ax.add_patch(FancyBboxPatch((318, y - 15), 104, 30, boxstyle="round,pad=0,rounding_size=8",
                                    facecolor=vc, edgecolor="none"))
        ax.text(370, y, vt, fontsize=14, fontweight="bold", color=ink_on(vc), va="center", ha="center")
        ax.text(438, y, f"20일 {d20*100:+.0f}%", fontsize=14, color=SUBINK, va="center")
        if lname:
            ax.text(566, y, lname, fontsize=16, color="#c8ccd6", va="center")
            ax.text(w - PAD, y, f"{lchg*100:+.1f}%", fontsize=16, fontweight="bold",
                    color=_num_ink(lchg * 100), va="center", ha="right")

    fig.savefig(out_path, facecolor=SURFACE, dpi=100)
    plt.close(fig)
    return out_path
